...
const showInfo = async (onu: Onu) => {
    ...
    try {
        ...
        if (response.data.status === 'success') {
            ...
            toast.error(
                response.data.message || t('onuTable.failedToFetchOnuInfo'),
            );
        }
    } catch (error: any) {
        toast.error(
            error.response?.data?.message || t('onuTable.failedToFetchOnuInfo'),
        );
    } finally {
    ...
    <div class="flex flex-col gap-4">
        <div class="flex items-center justify-between gap-4">
            <div class="relative max-w-sm flex-1">
                ...
                <Input
                    v-model="searchQuery"
                    :placeholder="t('onuTable.searchPlaceholder')"
                    class="pl-8"
                />
            </div>
        ...
        <!-- Not connected state -->
        <div
            v-if="!isConnected && !isScanning && onus.length === 0"
            class="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-sidebar-border/70 py-16 text-muted-foreground dark:border-sidebar-border"
        >
        ...
        >
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead>
                        <tr
                            class="border-b border-sidebar-border/70 bg-muted/50 transition-colors dark:border-sidebar-border"
                        >
                            <th v-if="auditSession" class="h-12 w-12 px-2">
                                <Checkbox
                                    :checked="
                                        selectedOnus.size === filtered.length &&
                                        filtered.length > 0
                                    "
                    ...
                    <tbody>
                        <tr
                            ...
                            class="border-b border-sidebar-border/70 transition-colors last:border-0 dark:border-sidebar-border"
                        >
                            <td
                                :colspan="auditSession ? 6 : 5"
                            ...
                            >
                                ...
                                <span v-else>{{
                                    t('onuTable.noUnconfiguredOnus')
                                }}</span>
                            </td>
                        </tr>
                        <tr
                            v-for="onu in filtered"
                            :key="onu.sn"
                        ...
                        >
                            ...
                            <td class="p-4 align-middle font-mono">
                                ...
                                <span
                                    v-if="isSaved(onu.sn)"
                                    class="ml-2 inline-flex items-center rounded-full bg-emerald-100 px-1.5 py-0.5 text-[10px] font-medium text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300"
                                >
        ...
        <div
            v-if="auditSession && props.onus.length > 0"
            class="flex items-center justify-between border-t border-sidebar-border/70 bg-muted/30 px-4 py-3"
        >
            <span class="text-sm text-muted-foreground">
                {{
                    selectedOnus.size > 0
                        ? t('onuTable.selectedCount', {
                              count: selectedOnus.size,
                          })
                        : t('onuTable.availableCount', {
                              count: props.onus.length,
                          })
                }}
            </span>
            <Button
                ...
                @click="
                    emit(
                        'save-to-session',
                        selectedOnus.size > 0
                            ? props.onus.filter((o) => selectedOnus.has(o.sn))
                            : props.onus,
                    )
                "
            >
        ...
        <Dialog v-model:open="isInfoOpen">
            <DialogContent class="max-w-lg">
                ...
                >
                    ...
                    <span class="text-sm">{{
                        t('onuTable.fetchDeviceInfo')
                    }}</span>
                </div>

                <div
                    v-else-if="Object.keys(onuDetail).length > 0"
                    class="mt-2 space-y-2 border-t pt-3"
                >
                    <h4
                        class="text-xs font-medium tracking-wider text-muted-foreground uppercase"
                    >
                        {{ t('onuTable.deviceDetails') }}
                    </h4>
                    <div
                        v-for="(value, key) in onuDetail"
                        :key="key"
                    ...
                    <div v-if="rawOutput" class="mt-3">
                        <details class="group">
                            ...
                            >
                                {{ t('onuTable.showRawOutput') }}
                            </summary>
                            <pre
                                class="mt-2 max-h-[200px] overflow-auto rounded-lg bg-slate-950 p-3 font-mono text-xs whitespace-pre-wrap text-emerald-400"
                                >{{ rawOutput }}</pre
                            >
                        </details>
                    </div>
                ...
                <div v-else-if="rawOutput" class="mt-2 border-t pt-3">
                    ...
                    >
                        {{ t('onuTable.rawOutput') }}
                    </h4>
                    <pre
                        class="max-h-[200px] overflow-auto rounded-lg bg-slate-950 p-3 font-mono text-xs whitespace-pre-wrap text-emerald-400"
                        >{{ rawOutput }}</pre
                    >
                </div>
            </DialogContent>
