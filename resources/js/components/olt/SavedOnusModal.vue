...
import { Printer, FileDown } from '@lucide/vue';
        ...
        <DialogContent class="max-w-2xl max-h-[85vh] flex flex-col">
            <DialogHeader>
                <DialogTitle class="flex items-center justify-between">
                    <span>{{ t('audit.bar.savedDataTitle') }} — {{ sessionName }}</span>
                </DialogTitle>
            </DialogHeader>
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    <Button variant="outline" size="sm" :disabled="onus.length === 0" @click="exportToCsv">
                        <FileDown class="mr-2 h-4 w-4" /> {{ t('onuTable.export') }}
                    </Button>
                    <Button variant="outline" size="sm" :disabled="onus.length === 0" @click="printTable">
                        <Printer class="mr-2 h-4 w-4" /> {{ t('onuTable.print') }}
                    </Button>
                </div>
            </div>

            <div class="rounded-xl border border-sidebar-border/70 dark:border-sidebar-border overflow-hidden flex-1 min-h-0">
